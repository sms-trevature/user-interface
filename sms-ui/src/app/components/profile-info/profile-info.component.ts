...
export class ProfileInfoComponent implements OnInit {
  ...
  someMethodToUpdateAddressOrUserInfo() {
    ...
    if (fnInput.value !== this.currentUser[0].firstName || lnInput.value !== this.currentUser[0].lastName
      || email.value !== this.currentUser[0].email || phoneN.value !== this.currentUser[0].phoneNumber) {
      userChange = true;
    ...
    if (addrs.value !== this.currentAddress[0].street || cty.value !== this.currentAddress[0].city ||
      stt.value !== this.currentAddress[0].state || cntry.value !== this.currentAddress[0].country ||
      zp.value !== this.currentAddress[0].zip || als.value !== this.currentAddress[0].alias) {
